...
class DevPanel extends StatelessWidget {
  ...
  ///   갱신할 때마다 글자 배치를 다시 잰다 — 켜고 볼 때만 켠다.
  /// ⚠️ 켜면 아이템을 먹는 순간 공이 밀렸던 것은 **디버그 모드에서 본 것**이다 (2026-09-11 profile 확인).
  final bool logOn;
  ...
  ///   예전에는 걸린 효과 수만큼 줄이 늘고 줄어, 효과가 붙고 떨어질 때마다
  ///   아래 EVENTS 칸이 위아래로 흔들렸다.
  ///   빈 슬롯은 자리만 차지하게 두면 **줄 수가 영영 변하지 않는다.**
  /// ⚠️ 처음에는 「첫 습득 때 공이 밀리는」 원인으로 보고 고쳤으나 **아니었다** —
  ///   고정한 뒤에도 남았고, 결국 **디버그 모드의 첫 실행 값**이었다 (2026-09-11 profile 확인).
  ///   배치가 흔들리지 않는 이점만으로도 남길 만해 그대로 둔다.
