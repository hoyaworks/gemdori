...
class BrickState {
  ...
  /// 2026-09-03 부터 **8종 전부** 대상 (예전에는 시간형을 뺐다)
  ItemType? flashItem;
  ...
  ///
  /// 상세 설명 : 속도는 speedTable 의 **자리 번호**다. 1~3 = 1(300) · 4~10 = 2(390) · 11 = 3(480).
  ...
  static const List<int> speedTable = [250, 300, 390, 480, 570];

  /// 속도 단계를 나타내는 기호 — 상단에 `SPEED (▲▲)` 처럼 상시 표시한다.
  /// 스테이지가 바뀔 때 기본 속도가 달라진 것도 이걸로 알 수 있다.
  ...
  void applyItem(ItemType t) {
    // 먹은 순간을 화면 가운데로 알린다 — **8종 전부** (2026-09-03 확대).
  ...
  //    목숨·점수 정합성은 신경 쓰지 않는다. 화면 하단 치트 버튼에서만 부른다.
